	...
	def train(self, mb_states, mb_actions, mb_old_values, mb_advs, mb_returns, mb_old_a_logps):
		...
		for i in range(self.sample_n_epoch):
			...
			for j in range(self.sample_n_mb):
				...
				pro_ratio = torch.exp(sample_a_logps) / torch.exp(sample_old_a_logps)
				pg_loss  = torch.min(pro_ratio * sample_advs, torch.clip(pro_ratio, 1 - self.clip_val, 1 + self.clip_val) * sample_advs).mean() * -1
