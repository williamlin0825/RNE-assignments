...
class EnvRunner:
	...
	def run(self, policy_net, value_net):
		...
		for step in range(self.n_step):
			#self.states : (n_env, s_dim) 8 * 14
			#actions     : (n_env, a_dim) 8 * 1
			#self.dones  : (n_env) 8
			#a_logps     : (n_env) 8
			#values      : (n_env) 8
			#rewards     : (n_env) 8
			...
			actions, log_probs = policy_net(state_tensor)
			self.mb_actions[step, :] = actions.cpu().numpy()
			self.mb_a_logps[step, :] = log_probs.cpu().numpy()
		...
		#mb_states : (n_step*n_env, s_dim) 128 * 8 * 14
		#mb_actions: (n_step*n_env) or (n_env*n_step, a_dim) 128 * 8
		#mb_a_logps: (n_step*n_env) 128 * 8
		#mb_values : (n_step*n_env)	128 * 8
		#mb_returns: (n_step*n_env)	128 * 8
