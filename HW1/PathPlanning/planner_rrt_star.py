...
class PlannerRRTStar(Planner):
    ...
    def planning(self, start, goal, extend_len=None, img=None):
        ...
        for it in range(20000):
            ...
            # Re-Parent
            temp_min = self.cost[new_node]
            temp_node = self.ntree[new_node]
            for n in self.ntree :
                if  utils.distance(n, new_node) <= 200 and not self._check_collision(n, new_node):
                    if utils.distance(n, new_node) + self.cost[n] < temp_min :  
                        temp_min = self.cost[n] + utils.distance(n, new_node)
                        temp_node = n

            self.ntree[new_node] = temp_node
            self.cost[new_node] = temp_min

            # Re-Wire
            for n in self.ntree :
                if utils.distance(n, new_node) <= 200 and not self._check_collision(n, new_node):
                    if self.cost[n] > self.cost[new_node] + utils.distance(n, new_node):
